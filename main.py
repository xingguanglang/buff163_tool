...
from finditem import generate_goods_url
from datetime import datetime
from time import time,sleep
filepath = "json/storage.json"
sleeptime=1
def generate_item_list_from_buff_item_list(buff_item:buff_item, selector:str = '#market-selling-list > tbody>.selling',usr_data_dir = "browser_buffer/chrome"):
...
def create_storage_item_main():
    value = []
    buff_item_list = read_buff_item_from_json()
    for i in buff_item_list:
        tmp = generate_item_list_from_buff_item_list(i)
        if tmp !=1:
            for j in tmp:
                value.append(j)
    ...
    write_item_to_json(result,filepath)
    print("finish")
    return
